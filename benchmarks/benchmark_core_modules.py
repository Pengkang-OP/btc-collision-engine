...
def benchmark_logger_performance(iterations=1000):
    ...
    print(f"  吞吐量: {iterations/total_time*1000:.0f} 条/秒")
    ...
    # 性能对比
    native_time = sum([t for t in times[:iterations]])  # 近似值
    threadsafe_time = total_time
    slowdown = (threadsafe_time / native_time - 1) * 100 if native_time > 0 else 0
