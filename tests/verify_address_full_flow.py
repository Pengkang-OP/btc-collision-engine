...
for fmt, addr in test_addrs.items():
    ...
    if resolved:
        check(f"解析 [{fmt}] → P2PKH: {resolved[:10]}...", resolved.startswith("1"),
              f"解析结果不是 P2PKH 地址: {resolved}")
    else:
        # P2SH和Taproot密码学上无法用于碰撞匹配，返回 None 是正确行为
        check(f"解析 [{fmt}] → None (设计行为)", fmt in ("p2sh_address", "taproot_address"),
              f"{fmt} 返回 None 但应可解析")
...
for fmt, addr in formats.items():
    check(f"私钥0x1 → {fmt}: {addr[:15]}...", addr is not None and len(addr) > 10,
          f"地址为空或过短")
...
check(f"WIF 解析为地址", resolved_from_wif is not None and resolved_from_wif.startswith("1"),
      f"解析失败: {resolved_from_wif}")

# 闭环验证: WIF→地址 == 私钥→地址
check(f"WIF解析地址 = 私钥生成地址",
...
check("check_match_all 返回正确类型", isinstance(matches, list),
      f"类型: {type(matches)}")
if matches and len(matches) > 0:
    known_formats = {"p2pkh", "p2sh", "bech32", "taproot"}
    for addr, fmt in matches:
        check(f"  匹配: {fmt} → {addr[:12]}...", fmt in known_formats,
              f"未知格式: {fmt}")
else:
...
check(f"目标地址: {list(target_addrs)[0][:15]}...", len(target_addrs) == 1,
      f"应有1个目标")
...
# 验证 0x1 产生的地址能匹配
check("私钥0x1 P2PKH 匹配", matcher.is_match(formats["p2pkh"]),
      f"地址 {formats['p2pkh']} 应匹配")
...
check("私钥0x2 P2PKH 不匹配", not matcher.is_match(formats_2["p2pkh"]),
...
check(f"私钥0x1 check_match={is_match_1} (地址: {addr_1})",
      is_match_1, f"0x1 应匹配, 实际: {is_match_1}")

is_match_2, addr_2, fmt_2 = manager.check_match(test_key_2)
check(f"私钥0x2 check_match={is_match_2} (地址: {addr_2})",
      not is_match_2, f"0x2 不应匹配, 生成的p2pkh地址: {formats_2['p2pkh']}")
...
try:
    ...
    check(f"文件加载地址数", file_targets is not None,
          f"加载失败: {file_targets}")

    if file_targets:
        # 应跳过无效地址和空行, 有效地址数
        valid_count = len(file_targets)
        check(f"有效地址数 >= 3", valid_count >= 3,
              f"期望 >= 3, 实际: {valid_count}")
        for addr in file_targets:
            check(f"  有效 P2PKH: {addr[:10]}...",
                  addr.startswith("1"),
