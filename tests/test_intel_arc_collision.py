...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler('intel_arc_collision_test.log', encoding='utf-8')
    ]
)
...
def load_valid_addresses(filepath: str = "valid_addresses.txt") -> Set[str]:
    """加载有效地址文件"""
    addresses = set()
    try:
...
class IntelArcTestMonitor:
    ...
    def record_batch(self, batch_size: int, execution_time_ms: float):
        ...
        # 记录显存（估算）
        estimated_memory = batch_size * 42 / 1048576  # 1M batch ≈ 42MB
        self.memory_monitor.track_allocation(
            int(estimated_memory * 1024 * 1024),
    ...
    def generate_report(self) -> str:
        ...
        report.append(f"  超时调整次数: {self.timeout_manager._timeout_adjustments}")
        report.append("")
        ...
        report.append(f"  状态: {mem_status['status'].value.upper()}")
        report.append("")
        report.append("=" * 80)
...
def run_gpu_collision_test(
    ...
    try:
        ...
        try:
            while (time.time() - start_time) < duration:
                ...
                if int(time.time() - start_time) % 5 == 0:
                    ...
                    # CollisionStats对象有speed和total_checked属性
        ...
        # 保存结果到文件
        result_file = f"intel_arc_test_result_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
...
def main():
    ...
    # 加载目标地址
    print("📂 加载目标地址...")
    targets = load_valid_addresses("valid_addresses.txt")
    
    if not targets:
        # 使用默认测试地址
        targets = {"12ib7dApVFvg82TXKycWBNpN8kFyiAN1dr"}
        print(f"  使用默认测试地址: {targets.pop()}")
        targets = {"12ib7dApVFvg82TXKycWBNpN8kFyiAN1dr"}
    else:
