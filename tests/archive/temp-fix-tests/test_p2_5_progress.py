...
class TestProgressCallbackControl(unittest.TestCase):
    ...
    def setUp(self):
        ...
        # 修复: 使用正确的初始化参数
        self.engine = KeyCollisionEngine(targets={"test_address"}, on_progress=None)  # Set[str]类型
...
class TestProgressCallbackIntegration(unittest.TestCase):
    ...
    def test_full_engine_with_progress(self):
        ...
        def on_progress(stats):
            progress_calls.append({"total_count": stats.total_count, "elapsed": stats.elapsed})

        # 创建引擎并设置回调
        engine = KeyCollisionEngine(targets={"test_address"}, on_progress=on_progress)
