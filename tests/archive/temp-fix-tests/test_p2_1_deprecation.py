...
class TestScalarMultiplyDeprecation(unittest.TestCase):
    ...
    def test_scalar_multiply_const_time_no_warning(self):
        ...
        with warnings.catch_warnings(record=True) as w:
            ...
            # 调用推荐的恒定时间方法
            result = self.ec.scalar_multiply_const_time(
                self.scalar, 
                self.generator
            )
    ...
    def test_both_methods_produce_same_result(self):
        ...
        with warnings.catch_warnings():
            ...
            result_old = self.ec.scalar_multiply(
                self.scalar, 
                self.generator
            )
        
        # 使用恒定时间方法
        result_new = self.ec.scalar_multiply_const_time(
            self.scalar, 
            self.generator
        )
    ...
    def test_deprecation_warning_stacklevel(self):
        ...
        with warnings.catch_warnings(record=True) as w:
            ...
            def wrapper_function():
                return self.ec.scalar_multiply(
                    self.scalar, 
                    self.generator
                )
