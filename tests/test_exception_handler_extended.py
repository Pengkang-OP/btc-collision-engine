...
class TestGPUCleanupError:
 ...
 def test_os_error(self):
     """OSError — 清理系统 I/O 错误"""
     ExceptionHandler.handle_gpu_cleanup_error(
     OSError("file handle closed"), "precomp_buffer"
     )
 ...
 # 所有路径都应正常处理，不抛异常
     try:
         ExceptionHandler.handle_gpu_cleanup_error(
         RuntimeError("error"), "test_buffer"
         )
...
class TestFileError:
 ...
 def test_permission_error(self):
     """PermissionError"""
     ExceptionHandler.handle_file_error(
     PermissionError("access denied"), "写入", "data.json"
     )
 ...
 def test_unknown_error(self):
     ...
     ExceptionHandler.handle_file_error(
     Exception("weird error"), "删除", "temp.txt"
     )
 ...
 def test_empty_operation(self):
     """空操作描述"""
     ExceptionHandler.handle_file_error(
     FileNotFoundError("missing"), "", "/path/to/file"
     )

 def test_empty_filepath(self):
     """空文件路径"""
     ExceptionHandler.handle_file_error(
     PermissionError("denied"), "读取", ""
     )
...
class TestConfigError:
 ...
 def test_value_error(self):
     """配置值无效"""
     ExceptionHandler.handle_config_error(
     ValueError("invalid port number"), "GPUConfig"
     )

 def test_type_error(self):
     """配置类型错误"""
     ExceptionHandler.handle_config_error(
     TypeError("expected dict"), "ConfigManager"
     )

 def test_permission_error(self):
     """配置文件权限不足"""
     ExceptionHandler.handle_config_error(
     PermissionError("access denied"), "CryptoConfig"
     )

 def test_unknown_error(self):
     """未知配置错误"""
     ExceptionHandler.handle_config_error(
     Exception("unknown config issue"), "ConfigManager"
     )
...
class TestEngineErrorEdge:
 ...
 def test_keyboard_interrupt_raises(self):
     ...
     with pytest.raises(KeyboardInterrupt):
         ...
         "CPU", KeyboardInterrupt(), context="扫描循环"
         )
 ...
 def test_os_error_classified(self):
     """OSError 被正确分类 (P3-6)"""
     ExceptionHandler.handle_engine_error(
     "GPU", OSError("device busy"), context="内核执行"
     )

 def test_empty_context(self):
     """空上下文"""
     ExceptionHandler.handle_engine_error(
     "CPU", RuntimeError("test"), context=""
     )

 def test_none_stats(self):
     """stats 为 None"""
     ExceptionHandler.handle_engine_error(
     "CPU", ValueError("test"), stats=None
     )
 ...
 # 不应抛出 AttributeError
     try:
         ExceptionHandler.handle_engine_error(
         "GPU", RuntimeError("test"), stats=stats
         )
...
class TestGPUErrorEdge:
 ...
 def test_always_returns_true(self):
     ...
     "随机碰撞", RuntimeError("test")
     )
     assert result is True

     result = ExceptionHandler.handle_gpu_error(
     "随机碰撞", Exception("unknown")
     )
     assert result is True
 ...
 def test_memory_error_classified(self):
     ...
     result = ExceptionHandler.handle_gpu_error(
     "范围扫描", MemoryError("out of memory")
     )
     assert result is True
 ...
 def test_overflow_error_classified(self):
     ...
     result = ExceptionHandler.handle_gpu_error(
     "随机碰撞", OverflowError("int too large")
     )
     assert result is True
 ...
 def test_none_stats(self):
     """stats 为 None"""
     result = ExceptionHandler.handle_gpu_error(
     "随机碰撞", RuntimeError("test"), stats=None
     )
     assert result is True
