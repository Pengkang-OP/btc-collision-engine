...
def main():
    ...
    has_nvidia = any('NVIDIA' in d.get('name', '') or 'GTX' in d.get('name', '') or 'RTX' in d.get('name', '') for d in devices)
    
    if has_intel_arc and has_nvidia:
        # 多GPU环境
        intel_arc_dev = next(d for d in devices if 'Arc' in d.get('name', ''))
        nvidia_dev = next(d for d in devices if 'NVIDIA' in d.get('name', '') or 'GTX' in d.get('name', '') or 'RTX' in d.get('name', ''))
        
        intel_score = calc_score(intel_arc_dev)
        nvidia_score = calc_score(nvidia_dev)
        
        print("多GPU环境检测:")
        print(f"  Intel Arc: {intel_arc_dev['name']} ({intel_arc_dev.get('global_mem_size', 0)/(1024**3):.1f}GB)")
        print(f"  NVIDIA: {nvidia_dev['name']} ({nvidia_dev.get('global_mem_size', 0)/(1024**3):.1f}GB)")
        print()
