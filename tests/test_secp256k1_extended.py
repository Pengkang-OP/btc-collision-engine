...
class TestModInverse(unittest.TestCase):
    ...
    def test_mod_inverse_basic(self):
        ...
        self.assertEqual(self.ec.mod_inverse(3, 7), 5)
    ...
    def test_mod_inverse_secp256k1(self):
        ...
        # 验证: (a * inv) % P = 1
        self.assertEqual((a * inv) % Secp256k1.P, 1)
    ...
    def test_mod_inverse_negative_input(self):
        ...
        self.assertEqual(self.ec.mod_inverse(-3, 7), 2)
...
class TestGeneratePublicKey(unittest.TestCase):
    """公钥生成测试"""
    ...
    def test_generate_public_key_const_time_default(self):
        ...
        # 验证x坐标一致
        self.assertEqual(pub_key[1:], expected_point.x.to_bytes(32, "big"))
...
class TestECPoint(unittest.TestCase):
    ...
    def test_is_infinity_flag(self):
        """无穷远点标志"""
        p1 = ECPoint(None, None)
        self.assertTrue(p1.is_infinity)

        p2 = ECPoint(100, 200)
        self.assertFalse(p2.is_infinity)
    ...
    def test_copy(self):
        ...
        p2 = p1.copy()
        self.assertEqual(p1, p2)
        self.assertIsNot(p1, p2)  # 不同的对象

    def test_repr_infinity(self):
        """无穷远点的字符串表示"""
        p = ECPoint(None, None)
        repr_str = repr(p)
        self.assertIn("Infinity", repr_str)

    def test_repr_normal(self):
        """普通点的字符串表示"""
        p = ECPoint(100, 200)
        repr_str = repr(p)
        self.assertIn("ECPoint", repr_str)
        self.assertIn("64", repr_str)  # 十六进制长度
