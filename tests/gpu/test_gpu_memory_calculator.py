...
@pytest.mark.unit
@pytest.mark.gpu
class TestCalculateBatchMemory:
    ...
    def test_large_batch_size(self):
        ...
        assert result == pytest.approx(expected_approx, abs=100_000)
...
@pytest.mark.unit
@pytest.mark.gpu
class TestCalculateBatchMemoryMb:
    ...
    def test_zero_inputs_returns_fixed_overhead(self):
        ...
        assert result == pytest.approx(expected, abs=0.00000000005)
    ...
    def test_consistent_with_byte_method(self):
        ...
        assert mb_result == pytest.approx(byte_result / (1024 * 1024), abs=0.00000000005)
    ...
    def test_one_mb_boundary(self):
        ...
        assert result == pytest.approx(1.002, abs=0.01)
...
class TestEstimateMaxBatchSize:
    ...
    def test_zero_targets_uses_full_remaining_memory(self):
        ...
        assert result_zero >= result_many
...
class TestGetMemoryBreakdown:
    ...
    def test_total_equals_sum_of_components(self):
        ...
        assert result["total_mb"] == pytest.approx(expected_total, abs=0.00000000005)
    ...
    def test_zero_inputs_all_zero(self):
        ...
        assert result["seed_buf_mb"] == pytest.approx(
            GPUMemoryCalculator.SEED_BUF_SIZE / bpMB, abs=1e-10
        )
        assert result["precomp_table_mb"] == pytest.approx(
            GPUMemoryCalculator.PRECOMP_TABLE_SIZE / bpMB, abs=1e-10
        )
        assert result["match_flags_mb"] == 0.0
        assert result["targets_mb"] == 0.0
        assert result["overhead_mb"] == 0.0
    ...
    def test_consistent_with_mb_method(self):
        ...
        assert breakdown["total_mb"] == pytest.approx(mb_method, abs=0.00000000005)
    ...
    def test_overhead_is_20_percent_of_match_flags(self):
        ...
        assert result["overhead_mb"] == pytest.approx(expected_overhead, abs=0.0000005)


@pytest.mark.unit
@pytest.mark.gpu
class TestCalculateFromHash160Bytes:
    ...
    def test_none_hash160_bytes_treats_as_zero_targets(self):
        ...
        assert result_none == pytest.approx(result_zero, abs=0.00000000005)
    ...
    def test_empty_bytes_equals_no_targets(self):
        ...
        assert result_empty == pytest.approx(expected, abs=0.00000000005)
