...
class TestGPUDeviceScorerScore:
    ...
    def test_01_score_nvidia_basic(self):
        ...
        assert s == pytest.approx(expected, abs=0.05)
    ...
    def test_04_score_with_cache_lmem(self):
        ...
        d = _make_device("Test GPU", "nvidia", 8.0, 32, cache_kb=128000, lmem_kb=65536)
        s = self.scorer.score(d)
...
class TestGPUDeviceScorerScoreRelative:
    ...
    def test_01_relative_uses_60_1_ratio(self):
        ...
        assert rel == pytest.approx(expected, abs=0.05)
    ...
    def test_03_relative_includes_vendor_factor(self):
        ...
        assert rel_nv == pytest.approx(84.8, abs=0.05)
        assert rel_amd == pytest.approx(55.86, abs=0.05)
...
class TestGPUDeviceScorerWeights:
    ...
    def test_01_normalized_weights_sum_to_one(self):
        ...
        assert sum(weights.values()) == pytest.approx(1.0, abs=0.000005)
...
class TestGPUDeviceScorerRankSelect:
    ...
    def test_02_select_best_returns_highest(self):
        ...
        assert best["name"] == "High"  # type: ignore[index]
...
class TestGPUDeviceScorerTier:
    ...
    def test_01_tier_boundaries(self):
        """等级分界线正确."""
        assert self.scorer.get_tier(200) == "S"
        assert self.scorer.get_tier(100) == "S"
        assert self.scorer.get_tier(99.9) == "A"
        assert self.scorer.get_tier(60) == "A"
        assert self.scorer.get_tier(59.9) == "B"
        assert self.scorer.get_tier(30) == "B"
        assert self.scorer.get_tier(29.9) == "C"
        assert self.scorer.get_tier(10) == "C"
        assert self.scorer.get_tier(9.9) == "D"
        assert self.scorer.get_tier(0) == "D"
        assert self.scorer.get_tier(-1) == "D"
...
class TestGPUDeviceScorerIdentifyModel:
    ...
    def test_01_nvidia_rtx50(self):
        assert self.scorer.identify_model("NVIDIA GeForce RTX 5090", "nvidia") == "rtx50"

    def test_02_nvidia_rtx40(self):
        assert self.scorer.identify_model("NVIDIA GeForce RTX 4090", "nvidia") == "rtx40"

    def test_03_nvidia_rtx30(self):
        assert self.scorer.identify_model("NVIDIA GeForce RTX 3080", "nvidia") == "rtx30"

    def test_04_nvidia_rtx20(self):
        assert self.scorer.identify_model("NVIDIA GeForce RTX 2080 Ti", "nvidia") == "rtx20"

    def test_05_nvidia_gtx16(self):
        assert self.scorer.identify_model("NVIDIA GeForce GTX 1660", "nvidia") == "gtx16"

    def test_06_nvidia_gtx10(self):
        assert self.scorer.identify_model("NVIDIA GeForce GTX 1080", "nvidia") == "gtx10"

    def test_07_nvidia_titan(self):
        assert self.scorer.identify_model("NVIDIA TITAN RTX", "nvidia") == "titan"

    def test_08_nvidia_tesla(self):
        assert self.scorer.identify_model("Tesla V100", "nvidia") == "tesla"

    def test_09_nvidia_quadro(self):
        assert self.scorer.identify_model("Quadro P4000", "nvidia") == "quadro"

    def test_10_nvidia_unknown(self):
        assert self.scorer.identify_model("NVIDIA SomeWeirdGPU", "nvidia") == "nvidia_other"

    # ── AMD RX 500/400 扩展 ──
    def test_11_amd_rx590(self):
        """S5 修复: RX 590 应识别为 rx500."""
        assert self.scorer.identify_model("Radeon RX 590", "amd") == "rx500"

    def test_12_amd_rx550(self):
        """S5 修复: RX 550 应识别为 rx500."""
        assert self.scorer.identify_model("Radeon RX 550", "amd") == "rx500"

    def test_13_amd_rx480(self):
        """S5 修复: RX 480 应识别为 rx500."""
        assert self.scorer.identify_model("Radeon RX 480", "amd") == "rx500"

    def test_14_amd_rx580(self):
        assert self.scorer.identify_model("Radeon RX 580", "amd") == "rx500"

    # ── AMD 其他 ──
    def test_15_amd_rx9000(self):
        assert self.scorer.identify_model("Radeon RX 9070 XT", "amd") == "rx9000"

    def test_16_amd_rx7000(self):
        assert self.scorer.identify_model("Radeon RX 7900 XTX", "amd") == "rx7000"

    def test_17_amd_rx6000(self):
        assert self.scorer.identify_model("Radeon RX 6800 XT", "amd") == "rx6000"

    def test_18_amd_rx5000(self):
        assert self.scorer.identify_model("Radeon RX 5700 XT", "amd") == "rx5000"

    def test_19_amd_vega(self):
        assert self.scorer.identify_model("Radeon RX Vega 64", "amd") == "vega"

    def test_20_amd_instinct(self):
        assert self.scorer.identify_model("AMD Instinct MI300X", "amd") == "instinct"

    def test_21_amd_unknown(self):
        assert self.scorer.identify_model("AMD SomethingStrange", "amd") == "amd_other"

    # ── Intel ──
    def test_22_intel_arc_bmg(self):
        assert self.scorer.identify_model("Intel Arc B580", "intel") == "arc_bmg"

    def test_23_intel_arc(self):
        assert self.scorer.identify_model("Intel Arc A770", "intel") == "arc"

    def test_24_intel_iris(self):
        assert self.scorer.identify_model("Intel Iris Xe", "intel") == "iris"

    def test_25_intel_unknown(self):
        assert self.scorer.identify_model("Intel Unknown GPU", "intel") == "intel_other"

    # ── Unknown ──
    def test_26_unknown_vendor(self):
        assert self.scorer.identify_model("Some Totally Unknown GPU", "unknown") is None
    ...
    def test_27_model_cache(self):
        ...
        assert cache_key in self.scorer._model_cache  # type: ignore[attr-defined]
        assert self.scorer._model_cache[cache_key] == "rtx40"  # type: ignore[attr-defined]
