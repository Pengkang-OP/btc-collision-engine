...
class TestConfigFilesQueueDepth(unittest.TestCase):
    ...
    def test_config_json_has_queue_depth(self):
        ...
        self.assertIn('queue_depth', gpu_cfg,
                      "config.json gpu 块应包含 queue_depth 字段")
    ...
    def test_gpu_block_validates_without_queue_depth_errors(self):
        ...
        queue_errors = {k: v for k, v in errors.items()
                        if 'queue_depth' in k}
        self.assertEqual(queue_errors, {},
                         f"config.intel_arc.json gpu 块 queue_depth 验证失败: {queue_errors}")
...
class TestAsyncGPUExecutorQueueDepth(unittest.TestCase):
    ...
    def test_initialize_buffers_creates_pool_matching_queue_depth(self):
        ...
        with patch.dict('sys.modules', {'pyopencl': mock_cl}), \
             patch('src.gpu.precompute.get_precomp_table',
                   return_value=np.zeros(496, dtype=np.uint32)):
            for qd in [1, 4, 8]:
                ...
                self.assertEqual(
                    len(ex._buffer_pool), qd,
                    f"queue_depth={qd} 时缓冲区池应有 {qd} 个缓冲区"
                )
...
class TestEngineQueueDepthInit(unittest.TestCase):
    ...
    def test_engine_reads_queue_depth_from_config(self):
        ...
        self.assertEqual(captured['queue_depth'], 7,
    ...
    def test_engine_uses_default_queue_depth_when_not_in_config(self):
        ...
        self.assertEqual(captured['queue_depth'], 4,
