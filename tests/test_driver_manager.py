...
class TestDriverVersionParser(unittest.TestCase):
    ...
    def test_parse_complex_version(self):
        """测试复杂版本号解析"""
        self.assertEqual(
            DriverVersionParser.parse_version("31.0.101.4500"),
            (31, 0, 101, 4500)
        )
    ...
    def test_parse_with_suffix(self):
        """测试带后缀的版本号"""
        self.assertEqual(
            DriverVersionParser.parse_version("520.67.03-beta"),
            (520, 67, 3)
        )
    ...
    def test_compare_versions(self):
        ...
        # 大于
        self.assertEqual(
            DriverVersionParser.compare_versions("520.00", "510.00"),
            1
        )
        # 等于
        self.assertEqual(
            DriverVersionParser.compare_versions("520.67", "520.67"),
            0
        )
        # 小于
        self.assertEqual(
            DriverVersionParser.compare_versions("510.00", "520.00"),
            -1
        )
    ...
    def test_compare_different_lengths(self):
        """测试不同长度版本号比较"""
        self.assertEqual(
            DriverVersionParser.compare_versions("520.67.03", "520.67"),
            1
        )
        self.assertEqual(
            DriverVersionParser.compare_versions("31.0.101", "31.0.101.4500"),
            -1
        )
    ...
    def test_is_version_compatible(self):
        """测试版本兼容性检查"""
        self.assertTrue(
            DriverVersionParser.is_version_compatible("520.00", "510.00")
        )
        self.assertTrue(
            DriverVersionParser.is_version_compatible("520.67", "520.67")
        )
        self.assertFalse(
            DriverVersionParser.is_version_compatible("500.00", "510.00")
        )
...
class TestDriverVersionEdgeCases(unittest.TestCase):
    ...
    def test_compare_with_build_number(self):
        """测试带编译号的版本比较"""
        self.assertEqual(
            DriverVersionParser.compare_versions(
                "31.0.101.4500",
                "31.0.101.4600"
            ),
            -1
    ...
    def test_health_check_intel_arc(self):
        ...
        # 应该包含Arc驱动更新频繁的建议
        self.assertTrue(
            any('Arc' in rec for rec in result['recommendations'])
        )
