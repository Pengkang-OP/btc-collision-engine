...
class TestDriverManager:
    ...
    def test_get_optimization_flags_new_driver(self):
        ...
        assert not flags["enable_fast_math"]  # 加密/哈希需严格精度
        assert flags["enable_shader_cache"]
...
class TestDriverVersionEdgeCases:
    ...
    def test_optimization_flags_nvidia_modern(self):
        ...
        assert flags["enable_async_compute"]
        assert flags["enable_shader_cache"]
        assert flags["enable_shader_reordering"]
        assert not flags["conservative_mode"]
...
class TestDriverCache:
    ...
    def test_driver_version_caching(self, mock_run, mock_platform):
        ...
        assert version1 == "520.67.03"
        assert mock_run.call_count == 1

        # 第二次调用 - 应该使用缓存
        version2 = DriverManager.detect_driver_version("NVIDIA")
        assert version2 == "520.67.03"
        assert mock_run.call_count == 1  # 不应该增加
    ...
    def test_clear_driver_cache(self):
        ...
        # 验证缓存已清空
        assert len(DriverManager._driver_version_cache) == 0
...
class TestUnstableDriverBlacklist:
    ...
    def test_unstable_driver_report(self):
        ...
        assert "total_unstable_versions" in report
        assert "vendors" in report
        assert "recommendations" in report

        assert report["last_updated"] == "2026-04-20"
        assert report["total_unstable_versions"] > 0
        assert len(report["vendors"]) == 3  # nvidia, amd, intel
...
class TestLinuxDriverDetection:
    ...
    def test_nvidia_linux_proc_detection(self, mock_run, mock_platform):
        ...
        version = DriverManager.detect_nvidia_driver_version()
        assert version == "520.67.03"
        assert mock_run.call_count == 2  # 尝试了两次
    ...
    def test_amd_linux_sysfs_detection(self, mock_run, mock_platform):
        ...
        assert "/sys/module/amdgpu/version" in str(mock_run.call_args)
