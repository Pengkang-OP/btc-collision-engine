...
class TestSecp256k1VerifyParameters(unittest.TestCase):
    ...
    def test_verify_parameters_P_leq_1(self):
        ...
        with patch.object(Secp256k1, "P", 0):
            self.assertFalse(Secp256k1.verify_parameters())
    ...
    def test_get_security_info(self):
        ...
        self.assertIn("parameters_verified", info)
...
class TestECPointEdge(unittest.TestCase):
    ...
    def test_eq_not_ecpoint(self):
        ...
        p = ECPoint(1, 2)
        self.assertFalse(p == "not_a_point")
    ...
    def test_repr_infinity(self):
        """无穷远点 repr → line 157"""
        p = ECPoint(None, None)
        self.assertIn("Infinity", repr(p))

    def test_repr_normal(self):
        """普通点 repr → line 158"""
        p = ECPoint(0xAB, 0xCD)
        r = repr(p)
        self.assertIn("ECPoint", r)
        self.assertIn("x=", r)
        self.assertIn("y=", r)
...
class TestPointAddEdge(unittest.TestCase):
    ...
    def test_p1_not_ecpoint(self):
        ...
        self.assertIn("p1必须是ECPoint", str(ctx.exception))

    def test_p2_not_ecpoint(self):
        """p2 非 ECPoint → TypeError → line 269"""
        with self.assertRaises(TypeError) as ctx:
            self.ec.point_add(self.G, 42)
        self.assertIn("p2必须是ECPoint", str(ctx.exception))
...
class TestIsOnCurveEdge(unittest.TestCase):
    """is_on_curve 边界"""
...
class TestConstTimeSelectEdge(unittest.TestCase):
    ...
    def test_select_normal_points(self):
        ...
        self.assertEqual(r0, a)
        # condition=1 → select b
        r1 = self.ec._const_time_select(1, a, b)
        self.assertEqual(r1, b)


class TestScalarMultiplyConstTimeEdge(unittest.TestCase):
...
class TestGeneratePublicKeyEdge(unittest.TestCase):
    ...
    def test_generate_public_key_infinity_raises(self):
        ...
        self.assertIn("无穷远点", str(ctx.exception))
