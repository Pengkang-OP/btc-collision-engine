...
GPU_SPECIFIC_CONFIG = types.MappingProxyType({
    ...
    "1660": {
        "queue_depth": 20,  # 翻倍，提高缓冲，消除GPU等待
        "initial_batch_size": 1048576,  # 8x提升，延长单次内核执行时间
        "max_batch_size": 2097152,  # 4x提升，允许更大的批次
        "memory_factor": 0.80,
    },
    # NVIDIA RTX 30系列
    "rtx30": {
...
class _SyncFallbackError(Exception):
    ...
    def __init__(self, matches: "list[tuple[bytes, list[dict]]]", execution_time_ms: float) -> None:
        super().__init__(
