...
class GPUDeviceManager:
    ...
    def __init__(self, device_index: int = -1, config: Optional[Dict[str, Any]] = None, logger: Optional[Any] = None) -> None:
        """
        Args:
    ...
    def initialize(self, targets: Set[str], batch_size: Optional[int] = None,
        ...
        with EnhancedPerformanceMonitor(self.logger, "GPU设备初始化", level="INFO") as pm:
            try:
                ...
                # 8. 设置目标地址
                if target_hash160s:
                    self._gpu_kernel.set_targets(target_hash160s, len(target_list),  # type: ignore[union-attr]
                    ...
                    try:
                        ...
                        with open(cfg_file, 'r', encoding='utf-8') as f:
                            ...
                            if 'async_execution' in gpu_cfg:
                                ...
                                self.logger.info(f"✅ 从{config_source}读取异步设置: {enable_async} (优先级2)")
                                break
    ...
    def _apply_vendor_optimizations(self):
        ...
        if vendor_lower.startswith('intel') or 'intel' in vendor_lower:
            ...
            try:
                ...
                self.logger.info(f"✅ NVIDIA 优化器已初始化: 架构={optimization_result.get('arch_name', 'Unknown')}, "
            ...
            try:
                ...
                self.logger.info(f"✅ AMD 优化器已初始化: 架构={optimization_result.get('arch_name', 'Unknown')}, "
                            f"memory_ratio={optimization_result.get('recommended_memory_ratio', 0.60):.2f}")
            except Exception as e:
