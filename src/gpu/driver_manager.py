...
class DriverManager:
    ...
    def get_unstable_driver_report() -> dict:
        ...
        return {
            ...
            "vendors": {
                vendor: len(versions) for vendor, versions in DriverManager.UNSTABLE_DRIVERS.items()
            },
            "recommendations": [
                "定期检查驱动更新",
                "避免使用黑名单中的驱动版本",
    ...
    def check_driver_health(vendor: str, driver_version: str, profile: dict | None = None) -> dict:
        ...
        if vendor_lower in DriverManager.UNSTABLE_DRIVERS:
            for min_ver, max_ver, issue in DriverManager.UNSTABLE_DRIVERS[vendor_lower]:
                if (
                    DriverVersionParser.is_version_compatible(driver_version, min_ver)
                    and DriverVersionParser.compare_versions(driver_version, max_ver) <= 0
                ):
                    result["status"] = "warning"
        ...
        if vendor_lower == "intel":
            ...
            result["recommendations"].append("Intel Arc驱动更新频繁,建议保持最新版本")
