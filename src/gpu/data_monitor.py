...
class DataMonitor:
    ...
    def get_stats(self) -> Dict:
        ...
        # 添加设备统计
        stats['devices'] = {}
        for device_idx, device_stats in self._device_stats.items():
    ...
    def _perform_checks(self):
        ...
        for device_idx in list(self._device_stats.keys()):
            # 检查数据是否过期
        ...
        if last_update > 0 and (current_time - last_update) > self.stale_data_timeout:
            issue = DataQualityIssue(
                ...
                details={
                    ...
                    'stale_duration': current_time - last_update
                }
            )
            self._record_issue(issue)
    ...
    def _detect_error_spike(self, device_idx: int):
        ...
        recent_errors = [
            ...
            if current_time - e['timestamp'] < 10
        ]
    ...
    def _record_issue(self, issue: DataQualityIssue):
        ...
        # 检查问题频率
        recent_issues = [
            t for t in self._issues_last_minute 
