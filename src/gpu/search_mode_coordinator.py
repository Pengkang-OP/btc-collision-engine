...
class SearchModeCoordinator:
    ...
    def _init_modes(self):
        ...
        cfg_gpu = self.engine.config.get('gpu', {}) if hasattr(self.engine, 'config') and self.engine.config else {}
        ...
        try:
            ...
            if hasattr(search_mode, 'get_status'):
                ...
                return {
                    "mode": mode,
                    "status": "available",
                    "engine_running": self.engine.is_running() if hasattr(self.engine, 'is_running') else False
                }
        except Exception as e:
