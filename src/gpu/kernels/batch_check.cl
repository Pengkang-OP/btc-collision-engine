...
#define HASH160_TARGET_SCAN(src_base, h0, h1, h2, h3, h4, n_targets, match) do {     /* v4.5: 溢出守卫 — num_targets 超过 ~214M 时地址计算溢出，拒绝扫描 */     if ((n_targets) > 214748364u) { (match) = 0; break; }     for (uint _t = 0; _t < (n_targets) && (match) == 0; _t++) {         const uchar *_src = (src_base) + (ulong)_t * 20u;         uint _t0 = (uint)_src[0]  | ((uint)_src[1]  << 8) | ((uint)_src[2]  << 16) | ((uint)_src[3]  << 24);         if (_t0 != (h0)) continue;         uint _t1 = (uint)_src[4]  | ((uint)_src[5]  << 8) | ((uint)_src[6]  << 16) | ((uint)_src[7]  << 24);         if (_t1 != (h1)) continue;         uint _t2 = (uint)_src[8]  | ((uint)_src[9]  << 8) | ((uint)_src[10] << 16) | ((uint)_src[11] << 24);         if (_t2 != (h2)) continue;         uint _t3 = (uint)_src[12] | ((uint)_src[13] << 8) | ((uint)_src[14] << 16) | ((uint)_src[15] << 24);         if (_t3 != (h3)) continue;         uint _t4 = (uint)_src[16] | ((uint)_src[17] << 8) | ((uint)_src[18] << 16) | ((uint)_src[19] << 24);         if (_t4 != (h4)) continue;         match = (int)(_t + 1);     } } while(0)
