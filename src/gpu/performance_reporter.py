...
class PerformanceReportGenerator:
    ...
    def _generate_json_report(self, config: ReportConfig) -> str:
        ...
        report_data = {
            ...
            'recommendations': self._generate_recommendations() if config.include_recommendations else None
        }
    ...
    def _get_device_info(self) -> Dict:
        ...
        if hasattr(self.gpu_engine, '_gpu_device') and self.gpu_engine._gpu_device:
            ...
            return {
                ...
                'OpenCL 版本': '3.0' if 'Intel' in str(device.device_info.get('vendor', '')) else 'Unknown'
            }
        return {}
    ...
    def _generate_recommendations(self) -> List[str]:
        ...
        if 'Intel' in vendor:
            ...
            recommendations.append(
                "Intel Arc GPU: 保持驱动版本 ≥ 31.0.101.4500"
            )
        
        # 性能建议
        if current_perf.get('throughput', 0) < 100000:
            recommendations.append(
                "吞吐量较低，建议运行自动调优以找到最优 batch_size"
            )
        ...
        if 'GB' in mem_size:
            try:
                ...
                if mem_gb >= 16:
                    recommendations.append(
                        "显存充足 (≥16GB)，可以尝试更大的 batch_size 提升吞吐量"
                    )
                elif mem_gb < 4:
                    recommendations.append(
