...
class GPUConfigManager:
    ...
    def prepare_config(self, device: Any, user_batch_size: int = None) -> Dict:
        """准备完整的GPU配置
    ...
    def _validate_config(self, config: Dict):
        ...
        if batch_size is not None:
            ...
            if batch_size > MAX_BATCH_SIZE_LIMIT:
                raise ValueError(
                    f"batch_size {batch_size} 超出最大限制 {MAX_BATCH_SIZE_LIMIT}"
                )
        ...
        if seed_prefetch_size is not None:
            ...
            if seed_prefetch_size > 100:
                self.logger.warning(f"seed_prefetch_size {seed_prefetch_size} 过大，可能导致内存使用过高")
        
        # 验证memory_ratio
        memory_ratio = config.get('memory_ratio')
        if memory_ratio is not None:
            if not isinstance(memory_ratio, (int, float)) or memory_ratio <= 0 or memory_ratio > 1.0:
