    ...
    def get_stats(self) -> Dict:
        ...
        with self._lock:
            ...
            if self._performance_history:
                avg_throughput = sum(r['throughput'] for r in self._performance_history) / len(self._performance_history)
                stats['avg_throughput'] = avg_throughput
            
            # 计算平均内存使用
            if self._memory_history:
                avg_usage = sum(r['usage_ratio'] for r in self._memory_history) / len(self._memory_history)
...
def get_batch_size_optimizer(initial_batch_size: int = 1048576, gpu_model: str = 'default') -> SmartBatchSizeOptimizer:
    """
    获取全局智能批次大小优化器实例
