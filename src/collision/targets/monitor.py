...
# 初始化日志系统
init_logging()
logger = get_configured_logger("ValidationMonitor", thread_safe=True)
...
class ValidationMonitor:
    ...
    def check_and_report(
        ...
        if not results:
            ...
            return ValidationMetrics(
                ...
                total=0, validated=0, unvalidated=0,
                coverage=0.0, valid=0, invalid=0, success_rate=0.0
            )
        ...
        if alert_level:
            ...
            if len(self.alert_history) > self.MAX_HISTORY_SIZE:
                ...
                logger.debug(
                    f"告警历史记录已清理: "
                    f"保留最近{self.HISTORY_TRIM_SIZE}条记录"
                )
            
            self.alert_callback(
                alert_level,
                self._format_alert_message(alert_level, metrics, batch_id),
                metrics
            )
    ...
    def _log_metrics(
        ...
        if alert_level == 'critical':
            logger.error(
                ...
                f"({metrics.unvalidated/metrics.total:.0%}" if metrics.total > 0 else "(0%)" 
                f"), "
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'alert_rate': (self.total_alerts / self.total_checks * 100) 
...
def create_monitor(
    ...
    thresholds = ValidationThresholds(
        ...
        unvalidated_warning=unvalidated_warning
    )
