    ...
    def _load_csv(self) -> Tuple[Set[str], Optional[Dict]]:
        ...
        try:
            ...
            if os.path.exists(metadata_path):
                metadata_content = EncodingUtils.read_file(metadata_path, encoding='utf-8', try_multiple=True)
    ...
    def import_addresses(self, source_path: str, storage_dir: str = None, 
        ...
        try:
            ...
            if storage_type == 'json':
                storage_path = os.path.join(storage_dir, f'imported_addresses_{timestamp}_{unique_id}.json')
            elif storage_type == 'sqlite':
                storage_path = os.path.join(storage_dir, f'imported_addresses_{timestamp}_{unique_id}.db')
            elif storage_type == 'csv':
                storage_path = os.path.join(storage_dir, f'imported_addresses_{timestamp}_{unique_id}.csv')
            else:
