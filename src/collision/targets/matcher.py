...
# 初始化日志系统
init_logging()
logger = get_configured_logger("AddressMatcher", thread_safe=True)
...
class AddressMatcher:
    ...
    def _init_bloom_filter(self, capacity: int, error_rate: float):
        ...
        try:
            ...
            logger.debug(f"布隆过滤器初始化成功: 容量={capacity}, 误判率={error_rate}, 目标数={len(self.targets)}")
        ...
        with self._lock:
            ...
            if self.strategy == 'hash_set':
                ...
                self._init_bloom_filter(
                    self._bloom.capacity,
                    self._bloom.error_rate
                )
            elif self.strategy == 'trie':
                self._trie.clear()
