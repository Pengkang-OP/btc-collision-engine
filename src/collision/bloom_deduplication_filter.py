...
class BloomFilter:
    ...
    def __init__(self, max_elements: int, false_positive_rate: float = 0.01):
        ...
        logger.info(f"Bloom Filter初始化: 容量={max_elements}, "
    ...
    def _hashes(self, item: bytes) -> list:
        ...
        h1 = int(hashlib.md5(item).hexdigest(), 16)  # nosec B324 - 仅用于Bloom过滤器性能优化
    ...
    def check(self, item: bytes) -> bool:
        ...
        return all(self.bit_array[hash_val] == 1 
                  for hash_val in self._hashes(item))
...
class BloomDeduplicationFilter:
    ...
    def __init__(self, max_size: int = 10_000_000, 
        ...
        logger.info(f"BloomDeduplicationFilter初始化: "
                   f"max_size={max_size}, "
    ...
    def get_stats(self) -> dict:
        ...
        return {
            ...
            "duplicate_rate": (self.duplicates_found / self.total_checks * 100 
