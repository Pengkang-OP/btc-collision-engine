    ...
    def handle_match(self, event: EngineMatchEvent) -> None:
        ...
        if self.data_logger:
            try:
                # 只记录脱敏地址，不记录私钥 (安全考虑)
                safe_addr = (
                    event.address[:6] + "..." + event.address[-4:]
                    if len(event.address) > 10
                    else event.address[:3] + "..."
                )
                safe_target = (
                    event.target_address[:6] + "..." + event.target_address[-4:]
...
def setup_enhanced_monitoring(
    event_bus: EventBus, monitoring_system: "EnhancedMonitoringSystem"
