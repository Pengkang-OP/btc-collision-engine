...
class OptimizationPerformanceMonitor:
    ...
    def __init__(self, 
        ...
        logger.info(f"OptimizationPerformanceMonitor初始化: "
    ...
    def record_metrics(self, 
        ...
        logger.debug(f"记录性能指标: speed={speed:.0f} addr/s, "
        ...
        with self._lock:
            ...
            report = {
                ...
                'optimization': {
                    ...
                    'precomputed_table_usage': sum(1 for m in self._metrics_history if m.precomputed_table),
                ...
                'memory': {
                    'current_mb': self._metrics_history[-1].memory_usage_mb if self._metrics_history else 0,
                    'average_mb': avg_memory
                },
                'latency': {
                    'avg_ms': self._metrics_history[-1].avg_generation_time_ms if self._metrics_history else 0,
                    'min_ms': self._metrics_history[-1].min_generation_time_ms if self._metrics_history else 0,
                    'max_ms': self._metrics_history[-1].max_generation_time_ms if self._metrics_history else 0
                },
    ...
    def _monitor_loop(self):
        ...
        while self._running:
            try:
                ...
                if current:
                    logger.debug(f"当前性能: {current.speed:.0f} addr/s, "
