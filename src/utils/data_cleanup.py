...
class DataCleaner:
    ...
    def clean_temp_files(self, max_age_days: int = 7, dry_run: bool = False) -> tuple[int, int]:
        ...
        logger.info(
            f"临时文件清理完成: 删除{files_removed}个文件, 释放{space_freed / 1024 / 1024:.2f}MB"
        )
    ...
    def clean_old_data(self, max_age_days: int = 30, dry_run: bool = False) -> tuple[int, int]:
        ...
        logger.info(
            f"过期数据清理完成: 删除{files_removed}个文件, 释放{space_freed / 1024 / 1024:.2f}MB"
        )
        return files_removed, space_freed
    ...
    def clean_monitoring_data(
        ...
        logger.info(
            f"监控数据清理完成: 删除{files_removed}个文件, 释放{space_freed / 1024 / 1024:.2f}MB"
        )
        return files_removed, space_freed
    ...
    def rotate_performance_log(self, max_size_mb: float = 10.0, dry_run: bool = False) -> bool:
        ...
        if file_size_mb <= max_size_mb:
            logger.debug(
                f"performance.log 大小为 {file_size_mb:.2f}MB，未超过阈值 {max_size_mb}MB，无需轮转"
            )
            return False

        logger.info(
            f"performance.log 大小为 {file_size_mb:.2f}MB，超过 {max_size_mb}MB 阈值，开始轮转..."
        )
    ...
    def clean_all(self, dry_run: bool = False) -> dict:
        ...
        tasks = [
            ...
            (
                "报告归档",
                lambda: (
                    (lambda r: (r["moved"], r["space_freed_bytes"]))(self.clean_old_reports())
                    if not dry_run
                    else (0, 0)
                ),
            ),
...
def main() -> None:
    ...
    if disk_usage:
        ...
        print(f"磁盘使用: {_used:.2f}GB / {_total:.2f}GB ({_pct:.1f}%)")
