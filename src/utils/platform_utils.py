...
class PlatformUtils:
    ...
    @classmethod
    def get_optimal_window_size(cls, root: Optional[tk.Tk] = None,
                                 width_ratio: float = 0.75,
                                 height_ratio: float = 0.80) -> Tuple[int, int]:
        """根据屏幕分辨率计算最佳窗口尺寸
        ...
        logger.info("最优窗口尺寸: %dx%d (屏幕: %dx%d)", 
    ...
    def scale_font_size(cls, size: int, root: Optional[tk.Tk] = None) -> int:
        ...
        if dpi_scale >= 1.5:
            ...
            logger.debug("字体缩放: %d -> %d (DPI: %.2f)", 
                        size, scaled_size, dpi_scale)
            return scaled_size
...
def get_platform_fonts() -> dict:
    ...
    """
    return {
        "ui": PlatformUtils.get_ui_font(),
        "mono": PlatformUtils.get_mono_font()
    }
