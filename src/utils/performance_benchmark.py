...
class BenchmarkResult:
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            'name': self.name,
            'config': self.config,
            'elapsed_seconds': self.end_time - self.start_time if self.end_time else 0,
...
class PerformanceBenchmark:
    ...
    def benchmark_simd_optimization(self, batch_sizes: List[int] = None):
        """测试SIMD优化性能
        ...
        for batch_size in batch_sizes:
            ...
            print(f"  SHA256: {result.avg_speed:,.0f} keys/s, "
                  f"耗时: {result.end_time - result.start_time:.3f}s")
            ...
            print(f"  Hash160: {result.avg_speed:,.0f} keys/s, "
                  f"耗时: {result.end_time - result.start_time:.3f}s")
            self.results.append(result)
    
    def benchmark_multiprocess(self, worker_counts: List[int] = None):
        """测试多进程性能
        ...
        for num_workers in worker_counts:
            ...
            class MockAddressGenerator:
                def generate_from_private_key(self, pk):
                    # 模拟地址生成
                    import hashlib
            ...
            print(f"  速度: {result.avg_speed:,.0f} keys/s, "
                  f"总检测: {result.total_processed:,}")
            self.results.append(result)
    ...
    def benchmark_bloom_filter(self, sizes: List[int] = None):
        """测试Bloom Filter性能
        ...
        for size in sizes:
            ...
            # 创建过滤器
            bloom = BloomDeduplicationFilter(
                max_size=size,
            ...
            # 测试添加性能
            result = BenchmarkResult(
                f"BloomFilter_Add_{size}",
                {'max_size': size, 'operation': 'add'}
            )
            result.start()
            ...
            print(f"  添加速度: {result.avg_speed:,.0f} items/s, "
                  f"耗时: {result.end_time - result.start_time:.3f}s")
    ...
    def benchmark_comparison(self):
        ...
        # 计算加速比
        speedup = result_simd.avg_speed / result_traditional.avg_speed if result_traditional.avg_speed > 0 else 0
        print(f"\n加速比: {speedup:.2f}x")
