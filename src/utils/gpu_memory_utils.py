...
class BatchSizeConfig:
    ...
    def validate(self):
        ...
        if not (0 < self.memory_usage_ratio <= 1.0):
            raise ValueError(
                f"memory_usage_ratio必须在(0, 1]范围内: {self.memory_usage_ratio}"
            )
        
        if self.min_batch_size <= 0:
            raise ValueError(
                f"min_batch_size必须为正数: {self.min_batch_size}"
            )
        ...
        if self.memory_alignment <= 0:
            raise ValueError(
                f"memory_alignment必须为正数: {self.memory_alignment}"
            )
