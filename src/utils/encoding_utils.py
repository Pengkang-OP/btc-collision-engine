...
class EncodingUtils:
    ...
    @staticmethod
    def detect_file_encoding(filepath: str, max_sample_size: Optional[int] = None, 
                            use_dynamic_sampling: bool = True) -> str:
        """
        检测文件编码
        ...
        try:
            ...
            if use_dynamic_sampling:
                ...
                if file_size < EncodingUtils.SMALL_FILE_THRESHOLD:
                    ...
                    actual_size = min(
                        max(int(file_size * 0.5), EncodingUtils.MIN_SAMPLE_SIZE),
                        actual_max_size
                    )
                    strategy = "50%采样"
                else:
                    ...
                    actual_size = min(
                        max(int(file_size * 0.1), EncodingUtils.MIN_SAMPLE_SIZE),
                        actual_max_size
                    )
                    strategy = "10%采样"
    ...
    @staticmethod
    def read_file(filepath: str, encoding: Optional[str] = None, 
                  try_multiple: bool = True) -> str:
        """
        读取文件内容，自动处理编码问题
    ...
    @staticmethod
    def read_file_lines(filepath: str, encoding: Optional[str] = None,
                       try_multiple: bool = True) -> List[str]:
        """
        读取文件行列表
    ...
    @staticmethod
    def write_file_lines(filepath: str, lines: List[str], encoding: str = 'utf-8',
                        ensure_dir: bool = True, newline: str = '\n') -> None:
        """
        写入行列表到文件
    ...
    @staticmethod
    def convert_file_encoding(src_path: str, dst_path: str, 
                             src_encoding: Optional[str] = None,
                             dst_encoding: str = 'utf-8') -> bool:
        """
        转换文件编码
