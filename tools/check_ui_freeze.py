...
def check_gui_process():
    ...
    print(f"OK 找到 {len(gui_processes)} 个GUI进程:")
    ...
    for log_file in log_files:
        if os.path.exists(log_file):
            ...
            print(f"  [MISS] {log_file} (不存在)")
    print()

    # 3. 检查checkpoint文件
    print("SAVE 检查checkpoint文件...")
    ...
    for ckpt_file in checkpoint_files:
        if os.path.exists(ckpt_file):
            ...
            try:
                ...
                status = "OK" if time_ago.total_seconds() < 10 else "WARN"
                ...
                print(f"  WARN {ckpt_file} (读取失败: {e})")
        else:
            print(f"  [MISS] {ckpt_file} (不存在)")
    print()
    ...
    if most_recent_log:
        ...
        if time_ago.total_seconds() < 30:
            ...
            print()
            print("TIP 提示: 如果界面无响应，但文件持续更新，可能是:")
