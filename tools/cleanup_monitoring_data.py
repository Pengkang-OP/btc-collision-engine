...
class MonitoringDataCleaner:
    ...
    def __init__(self, monitoring_dir: str = "monitoring_data", 
    ...
    def cleanup_old_files(self, max_age_days: int = 30, dry_run: bool = False) -> dict:
        ...
        if os.path.exists(logs_dir):
            stats["logs"] = self._cleanup_log_files(
                logs_dir, max_age_days, dry_run
            )
    ...
    def _cleanup_directory(self, directory: str, cutoff_time: float, 
    ...
    def _cleanup_log_files(self, logs_dir: str, max_age_days: int, 
                          dry_run: bool = False) -> dict:
        """
        清理日志文件（包括轮转的日志）
        ...
        try:
            for filename in os.listdir(logs_dir):
                ...
                if file_mtime < cutoff_time:
                    ...
                    if dry_run:
                        logger.info(f"[试运行] 将删除日志: {filename} ({file_size/1024/1024:.1f} MB)")
                        stats["skipped_files"] += 1
                    else:
                        try:
                            os.remove(filepath)
...
def main():
    ...
    if args.recommend:
        ...
        stats = cleaner.cleanup_old_files(
