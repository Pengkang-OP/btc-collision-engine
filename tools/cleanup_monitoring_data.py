...
class MonitoringDataCleaner:
    ...
    def __init__(self, monitoring_dir: str = "monitoring_data", data_logs_dir: str = "data_logs"):
        """
        初始化清理器

        Args:
            monitoring_dir: 监控数据目录
            data_logs_dir: 数据日志目录
        """
        self.base_dir = os.path.dirname(os.path.abspath(__file__))
        self.monitoring_dir = os.path.join(self.base_dir, "..", "..", monitoring_dir)
        self.data_logs_dir = os.path.join(self.base_dir, "..", "..", data_logs_dir)

        logger.info("监控数据清理器初始化完成")
        logger.info(f"监控数据目录: {self.monitoring_dir}")
        logger.info(f"数据日志目录: {self.data_logs_dir}")
    ...
    def cleanup_old_files(self, max_age_days: int = 30, dry_run: bool = False) -> dict:
        ...
        if os.path.exists(logs_dir):
            stats["logs"] = self._cleanup_log_files(logs_dir, max_age_days, dry_run)
    ...
    def _cleanup_directory(self, directory: str, cutoff_time: float, dry_run: bool = False) -> dict:
        """
        清理指定目录
        ...
        try:
            for filename in os.listdir(directory):
                ...
                if file_mtime < cutoff_time:
                    ...
                    if dry_run:
                        logger.info(f"[试运行] 将删除: {filename} ({file_size / 1024:.1f} KB)")
                        ...
                        try:
                            ...
                            logger.info(f"已删除: {filename} ({file_size / 1024:.1f} KB)")
                        except Exception as e:
                            error_msg = f"删除失败 {filename}: {e}"
    ...
    def _cleanup_log_files(self, logs_dir: str, max_age_days: int, dry_run: bool = False) -> dict:
        """
        清理日志文件（包括轮转的日志）
        ...
        try:
            for filename in os.listdir(logs_dir):
                ...
                if file_mtime < cutoff_time:
                    ...
                    if dry_run:
                        logger.info(
                            f"[试运行] 将删除日志: {filename} ({file_size / 1024 / 1024:.1f} MB)"
                        )
                        stats["skipped_files"] += 1
                    else:
                        try:
                            ...
                            stats["deleted_files"] += 1
                            stats["freed_space_bytes"] += file_size
                            logger.info(f"已删除日志: {filename} ({file_size / 1024 / 1024:.1f} MB)")
                        except Exception as e:
                            error_msg = f"删除日志失败 {filename}: {e}"
                            stats["errors"].append(error_msg)
                            logger.error(error_msg)
                else:
                    stats["skipped_files"] += 1
        except Exception as e:
            error_msg = f"清理日志目录失败: {e}"
            stats["errors"].append(error_msg)
            logger.error(error_msg)
    ...
    def get_cleanup_recommendation(self, max_age_days: int = 30) -> dict:
        ...
        logs_dir = os.path.join(self.base_dir, "..", "..", "logs")
        logs_size = self.get_directory_size(logs_dir) if os.path.exists(logs_dir) else 0
