...
def detect_language_v2(code_content: str, context_before: str, context_after: str) -> str:
    ...
    # 1. Mermaid图表
    if re.search(r'^(graph|sequenceDiagram|classDiagram|stateDiagram|gantt|pie)', code_content, re.MULTILINE):
        return 'mermaid'
    ...
    yaml_match_count = sum(1 for pattern in yaml_patterns if re.search(pattern, code_content, re.MULTILINE))
    ...
    md_match_count = sum(1 for pattern in markdown_patterns if re.search(pattern, code_content, re.MULTILINE))
    if md_match_count >= 2:
        return 'markdown'
