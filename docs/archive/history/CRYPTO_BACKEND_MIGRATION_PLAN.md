...
crypto_backend.py (抽象层)
    ...
    ├── CoincurveBackend (libsecp256k1) [OK_CHECK] 推荐
...
1. [OK_CHECK] `src/collision/key_collision_engine.py` - CPU碰撞引擎
2. [OK_CHECK] `src/collision/gpu_collision_engine.py` - GPU碰撞引擎  
3. [OK_CHECK] `src/collision/plugins/example_plugin.py` - 示例插件

**已使用crypto_backend的文件** (无需迁移):

1. [OK_CHECK] `src/core/address_generator.py` - 地址生成器
2. [OK_CHECK] `src/config/crypto_config.py` - 加密配置
...
**当前环境**: coincurve已安装并自动启用 [OK_CHECK]
...
| 恒定时间 | [CROSS] 无法保证 | [OK_CHECK] 保证 | **安全** |
| 侧信道防护 | [CROSS] 无 | [OK_CHECK] 有 | **安全** |

### 兼容性目标

- [OK_CHECK] 保持向后兼容（降级到PurePythonBackend）
- [OK_CHECK] 统一错误处理
- [OK_CHECK] 保持API一致性
- [OK_CHECK] 测试覆盖率100%
...
1. [OK_CHECK] coincurve后端可用性检查
2. [OK_CHECK] 性能对比测试（coincurve vs secp256k1.py）
3. [OK_CHECK] 计算正确性验证
4. [OK_CHECK] 降级机制测试
5. [OK_CHECK] 线程安全测试
