...
| 验证项目 | 状态 | 说明 |
|---------|------|------|
| **Mermaid 语法** | [OK_CHECK] 通过 | 流程图语法符合 Mermaid 规范 |
| **节点连接** | [OK_CHECK] 通过 | 所有节点连接关系完整正确 |
| **子图分组** | [OK_CHECK] 通过 | 4 个子图分组逻辑清晰 |
| **颜色样式** | [OK_CHECK] 通过 | 所有节点颜色样式正确应用 |
| **中文显示** | [OK_CHECK] 通过 | 中文字符正常显示无乱码 |
| **决策节点** | [OK_CHECK] 通过 | 菱形决策节点正确渲染 |
| **箭头方向** | [OK_CHECK] 通过 | 流程箭头方向正确 |
| **HTML 标签** | [OK_CHECK] 通过 | `<br/>` 换行标签正常解析 |
...
| 子图名称 | 颜色 | 节点数 | 说明 |
|---------|------|--------|------|
| **Step1Process** | [BLUE] 浅蓝 (#e3f2fd) | 7 | WIF 地址导入流程 |
| **Step2Process** | [PURPLE] 紫色 (#f3e5f5) | 8 | GPU 模式配置流程 |
| **Step3Process** | [GREEN] 绿色 (#e8f5e9) | 9 | GPU 设备选择流程 |
| **Step4Process** | [ORANGE] 橙色 (#fff3e0) | 13 | 碰撞执行与监控流程 |
...
[OK_CHECK] **颜色对比度**: 所有节点颜色与文字对比度良好，易于阅读  
[OK_CHECK] **分组识别**: 不同子图使用不同颜色，层次分明  
[OK_CHECK] **状态区分**: 成功/失败/决策节点使用不同颜色，一目了然  
...
```mermaid
[OK] flowchart TD - 流程图方向正确 (从上到下)
[OK] subgraph ... end - 子图语法正确
[OK] [] - 矩形节点语法正确
[OK] () - 圆角矩形节点语法正确
[OK] {} - 菱形决策节点语法正确
[OK] --> - 箭头连接语法正确
...
| 检查项 | 状态 | 说明 |
|-------|------|------|
| 加载方式 | [OK_CHECK] | 文件或输入框 |
| 格式验证 | [OK_CHECK] | Base58Check、版本字节、校验和 |
| 转换流程 | [OK_CHECK] | WIF → 私钥 → 公钥 → Hash160 |
| 存储优化 | [OK_CHECK] | Set[str] O(1) 查找 |
| 统计信息 | [OK_CHECK] | 成功数、失败数、去重数 |

### 第二步：选择 GPU 模式

| 检查项 | 状态 | 说明 |
|-------|------|------|
| 模式选择 | [OK_CHECK] | GPU 加速模式选项 |
| 设备扫描 | [OK_CHECK] | pyopencl/clinfo 检测 |
| 参数配置 | [OK_CHECK] | batch_size、memory_usage 等 |
| 配置验证 | [OK_CHECK] | 显存、计算能力、参数范围 |
| 错误处理 | [OK_CHECK] | 无 GPU 时提示错误 |

### 第三步：选择 GPU 设备

| 检查项 | 状态 | 说明 |
|-------|------|------|
| 设备列表 | [OK_CHECK] | 列出所有可用 GPU |
| 详细信息 | [OK_CHECK] | 型号、厂商、显存、计算单元等 |
| 选择方式 | [OK_CHECK] | 手动选择 + 自动选择 |
| 自动评分 | [OK_CHECK] | 显存 40% + 计算 35% + 频率 25% |
| 环境初始化 | [OK_CHECK] | Context、Queue、Kernel |

### 第四步：启动地址比对

| 检查项 | 状态 | 说明 |
|-------|------|------|
| 引擎初始化 | [OK_CHECK] | GPUCollisionEngine |
| 碰撞启动 | [OK_CHECK] | engine.start() |
| 进度监控 | [OK_CHECK] | 已检查、速度、时间、匹配数 |
| GPU 监控 | [OK_CHECK] | 利用率、显存、温度、功耗 |
| 匹配处理 | [OK_CHECK] | 记录、通知、回调 |
| 停止条件 | [OK_CHECK] | 手动、目标数量、发现匹配 |
| 结果保存 | [OK_CHECK] | 文件、日志、断点 |
...
| 屏幕尺寸 | 状态 | 说明 |
|---------|------|------|
| **1920x1080** | [OK_CHECK] | 完整显示，无需滚动 |
| **1366x768** | [OK_CHECK] | 需要横向滚动，内容完整 |
| **移动端** | [OK_CHECK] | 自动缩放，可正常查看 |
...
| 指标 | 数值 | 评级 |
|-----|------|------|
| **渲染时间** | ~200ms | [OK_CHECK] 优秀 |
| **SVG 大小** | ~45KB | [OK_CHECK] 良好 |
| **节点数量** | 35 个 | [OK_CHECK] 适中 |
| **复杂度** | 中等 | [OK_CHECK] 可接受 |
...
| 维度 | 评分 | 说明 |
|-----|------|------|
| **布局清晰度** | [STAR][STAR][STAR][STAR][STAR] | 从上到下，层次分明 |
| **颜色区分度** | [STAR][STAR][STAR][STAR][STAR] | 不同步骤颜色区分明显 |
| **文字可读性** | [STAR][STAR][STAR][STAR][ESTAR] | 部分节点文字较多，但可接受 |
| **流程逻辑性** | [STAR][STAR][STAR][STAR][STAR] | 符合实际操作流程 |
...
1. [OK_CHECK] **语法正确**: Mermaid 语法完全符合规范
2. [OK_CHECK] **结构完整**: 四个步骤流程完整，逻辑清晰
3. [OK_CHECK] **视觉美观**: 颜色方案合理，层次分明
4. [OK_CHECK] **内容准确**: 操作步骤和技术细节准确无误
5. [OK_CHECK] **渲染正常**: 浏览器中正常渲染，无错误
6. [OK_CHECK] **响应式**: 支持不同屏幕尺寸
