...
| 文件 | 豁免规则 | 评估 |
|------|----------|------|
| `constants.py` | `# noqa: F401, E402` | ✅ 合理 — 重导出模块需要 |
| `__init__.py` | `# noqa: F401` | ✅ 合理 — 版本导入需要 |
| `event_bus.py` | `# noqa: PLC0415` | ✅ 合理 — 延迟导入避免循环依赖 |
| `events.py` | `# noqa: I001` (推测) | ⚠️ 需要验证是否有实际违规 |
...
- ✅ 标准库 → 第三方 → 本地导入的 isort 风格基本遵循
- ⚠️ `event_bus.py` 在函数内部使用 `from .events import ...` 延迟导入 — 合理但属于例外

#### 2.1.4 PEP8 命名

- ✅ 类名: PascalCase（`KeyCollisionEngine`, `EventBus` 等）
- ✅ 方法/变量: snake_case
- ✅ 私有方法: `_` 前缀
- ⚠️ `CollisionEvent` 类名与模块名 `collision` 冲突，容易混淆（`events.py:159`）
...
| 文件 | 行数 | 评估 |
|------|------|------|
| `key_collision_engine.py` | **~2,452** (97KB) | ❌ **严重超标** — 建议拆分至 ≤1,000 行 |
| `collision_stats.py` | **435** | ⚠️ 偏大但尚可 |
| `targets/storage.py` | **684** | ⚠️ 偏大，可考虑拆分 storage 和 export |
| `targets/resolver.py` | **~650** | ⚠️ 偏大 |
| `event_bus.py` | 253 | ✅ 合理 |
...
| 函数 | 位置 | 行数 | 评估 |
|------|------|------|------|
| `__init__` | `key_collision_engine.py` | ~200+ | ❌ 过长，承担过多初始化职责 |
| `_random_search_worker()` | 同上 | ~160 | ⚠️ 偏大 |
| `_worker_process_key()` | 同上 | ~140 | ⚠️ 偏大 |
| `start()` | 同上 | ~60 | ⚠️ 较大 |
| `_validate_single()` | `targets/validator.py` | 隐式 | ⚠️ 只有基本格式检查 |
...
| 模式 | 位置 | 评估 |
|------|------|------|
| Singleton | `event_bus.py:get_event_bus()` | ✅ 合理，但全局状态不利于测试 |
| Adapter/Facade | `gpu/facade.py` | ✅ 合理封装 GPU 复杂性 |
| Strategy | `targets/matcher.py` (3种匹配策略) | ✅ 好的回退设计 |
| Template Method | `base_engine.py` (ABC) | ✅ 合理的基类设计 |
...
| 决策 | 评估 |
|------|------|
| **事件驱动架构** (EventBus) | ✅ 合理解耦组件 |
| **检查点原子写入** (.json.tmp + replace) | ✅ 防止写入中断导致数据损坏 |
| **CRC32 校验** | ✅ 适合完整性检查（非安全场景） |
| **MultiProcessEngine 独立存在** | ❌ **大问题** — `_process_task()` 仅把 task 放回 result queue，没有任何实际密钥生成或碰撞检测逻辑，是存根实现（`multiprocess_engine.py:94-107`） |
| **`constants.py` 重导出** | ✅ 合理的向后兼容方式 |

#### 2.3.2 算法选择

| 算法 | 位置 | 评估 |
|------|------|------|
| set membership 匹配 | `key_collision_engine.py` | ✅ 对于 hash160 集合查找是最优选择 |
| Bloom Filter 回退 | `targets/matcher.py` | ✅ 好的内存优化，自动 fallback |
| LRU + TTL 缓存 | `targets/cache.py` | ✅ 合理，但 `__bool__` 总是返回 True 可疑 |
| SHA256(private_key) 部分哈希 | `collision_stats.py:143` | ✅ 合理的隐私保护（存储 hash 而非明文字段） |

#### 2.3.3 依赖管理

| 依赖 | 文件 | 评估 |
|------|------|------|
| `cachetools` | `targets/cache.py` | ⚠️ 可选依赖 — 缺失时静默回退到 `dict`，失去 LRU 语义 |
| `pybloom_live` | `targets/matcher.py` | ⚠️ 仅在 `TYPE_CHECKING` 中导入 |
| `win32security` | `checkpoint_manager.py` | ❌ **不可达代码** — `_check_win32_security()` 从未被调用 |
...
| 检查 | 结果 |
|------|------|
| `k < 1 or k >= Secp256k1.N` 边界检查 | ✅ 所有扫描模式都正确校验 |
| 地址匹配算法（压缩/非压缩） | ✅ 先检查压缩地址，再按 `check_uncompressed` 决定是否检查非压缩 |
| CRC32 双序列化计算 | ✅ 先序列化计算 CRC，再追加 CRC 后第二次序列化写入，正确 |
| 进度报告 `_live_range_count` 递增/递减逻辑 | ✅ `_range_scan_worker` 每32个递增，`range_scan` 完成时递减并计入 total_count |

#### 2.4.2 竞态条件风险

| 位置 | 风险 | 评估 |
|------|------|------|
| `collision_stats.py` 所有属性 | 使用 threading.Lock | ✅ 充分防护 |
| `event_bus.py` publish | 全局锁保护订阅者列表 | ✅ 充分 |
| `key_collision_engine.py` stop() | `_stop_send_signals` 先设信号再 join | ✅ 正确的顺序 |
| `key_collision_engine.py` start() | `if self._running: return` 非原子检查 | ⚠️ 两次快速调用 start() 可能竞态进入 |
| `event_bus.py:get_event_bus()` | Double-checked locking | ✅ 正确实现 |
...
| 文件 | 行 | 上下文 | 评估 |
|------|----|--------|------|
| `event_bus.py:61-64` | `_subscribers: dict[Any, list[Callable]]` | 订阅者字典 | ❌ 应使用 `dict[EventType | type[EngineEvent], ...]` |
| `event_bus.py:66` | `_all_subscribers: list[Callable]` | 未指定参数类型 | ⚠️ 应指定 `Callable[[Any], None]` 或更具体 |
| `event_bus.py:75` | `subscribe(event_type: Any, handler: Callable)` | 参数 | ❌ `event_type` 收窄为 `EventType | type` |
| `base_engine.py:48` | `get_stats() -> Any` | 返回值 | ❌ 应返回 `CollisionStats` |
| `collision_stats.py:251` | `update(self, total_checked=0, **kwargs: Any)` | kwargs | ⚠️ 可定义 `TotalRange` 协议 |
| `events.py:26` | `to_dict() -> dict[str, Any]` | 返回值 | ✅ 合理（字典值类型多样）|
| `events.py:168` | `event_type: Any = None` | 属性 | ❌ 应允许 `EventType \| None` |

#### 2.5.2 `# type: ignore` 使用统计

| 文件 | 行 | 原因 | 是否仍需豁免 |
|------|----|------|-------------|
| `events.py:31` | `asdict(self)  # pyright: ignore[reportArgumentType]` | dataclass 的 asdict 类型混淆 | ⚠️ 这是 pyright 专用指令，对 mypy 无效 |
| `__init__.py:7` | `from src import __version__  # noqa: F401` | F401 豁免 | ✅ 需要（导入版本号但不直接使用） |
...
| 文件 | 定义 __all__? | 评估 |
|------|---------------|------|
| `collision/__init__.py` | ✅ 4 项 | ✅ 好 |
| `targets/__init__.py` | ✅ 6 项 | ✅ 好 |
| `gpu/__init__.py` | ✅ 18 项 | ✅ 好 |
| 其余 19 个文件 | ❌ 未定义 | **Major** — 缺少模块导出规范 |

#### 2.5.4 类型注解覆盖度

| 文件 | 评估 |
|------|------|
| `types.py` | ❌ 使用 `Callable[..., None]` — `...` 使类型检查几乎无效 |
| `key_collision_engine.py` | ⚠️ 类级别注解丰富，但内部方法很多无注解 |
| `collision_stats.py` | ⚠️ 大部分有注解，但 `eta_seconds` 返回 `float` 且三种值混用 |
| `events.py` | ✅ 良好的 dataclass 注解，但 `CollisionEvent` 使用了 `Any` |
| `checkpoint_manager.py` | ✅ 基本完整 |
...
| 检查 | 文件 | 结果 |
|------|------|------|
| 密钥范围验证 | `key_collision_engine.py` (所有 worker) | ✅ `k < 1 or k >= Secp256k1.N` 正确 |
| 地址匹配（hash160 set 查找） | `key_collision_engine.py` | ✅ O(1) 时间复杂度，正确 |
| WIF 编码异常处理 | `key_collision_engine.py:1500-1559` | ✅ wrapped in try/except (ValueError, TypeError, OverflowError) |
| 检查点敏感字段剥离 | `checkpoint_manager.py:183-185` | ✅ `private_key_hex` 和 `private_key_wif` 被 pop |
| EventBus 不传递密文私钥 | `key_collision_engine.py:1514-1523` | ✅ WIF 被设为空字符串，私钥设为空 bytes |
| 事件中 WIF 自动掩码 | `events.py:87-91` | ✅ `__post_init__` 自动掩码为 `6chars...4chars` |

#### 2.6.2 输入验证

| 位置 | 验证 | 结果 |
|------|------|------|
| `start()` 参数验证 | `key_collision_engine.py:2085-2102` | ✅ 检查 mode 有效性、range 参数完整性、整数类型 |
| 目标地址验证 | `targets/validator.py` | ⚠️ 仅检查 format prefix + length，无完整密码学验证 |
| SQL 注入防护 | `targets/storage.py` | ✅ 使用参数化查询 |
| 路径穿越防护 | `targets/storage.py` | ✅ 标准化路径检查 |

#### 2.6.3 时序侧信道风险

| 位置 | 风险 | 评估 |
|------|------|------|
| 所有 hash160 比较 | `hash160 in set[Hashes]` (Python set) | ✅ 非定常比较，但 hash160 是公开中间值，无秘密依赖 |
| 不存在显式的秘密依赖条件分支 | 各扫描 worker | ✅ 密钥比较不依赖秘密值 |
| WIF 编码错误处理 | `_worker_check_and_handle_match` | ✅ 不泄漏私钥信息 |
