...
set "LOGGING_MODULE=src.logging.log_manager"
...
| 日志模式 | `python -m src.logging.log_manager --watch` | 仅运行日志监控 |
...
1. **模块化设计**：创建独立的 `src/wizard/` 和 `src/logging/` 目录
