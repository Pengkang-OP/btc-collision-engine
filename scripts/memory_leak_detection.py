...
def test_gpu_collision_engine_memory_leak():
    ...
    logger.info(f"初始内存使用: RSS={initial_memory['rss']:.2f}MB, VMS={initial_memory['vms']:.2f}MB, 使用率={initial_memory['percent']:.2f}%")
    ...
    for i in range(5):
        ...
        logger.info(f"循环 {i+1} 内存使用: RSS={current_memory['rss']:.2f}MB, VMS={current_memory['vms']:.2f}MB, 使用率={current_memory['percent']:.2f}%")
    ...
    logger.info(f"最终内存使用: RSS={final_memory['rss']:.2f}MB, VMS={final_memory['vms']:.2f}MB, 使用率={final_memory['percent']:.2f}%")
...
def test_multi_gpu_engine_memory_leak():
    ...
    for i in range(3):
        ...
        try:
            ...
            start_result = engine.start(targets=test_targets, mode='random', total_keys=10000000)  # 1000万次碰撞检测
            if not start_result:
