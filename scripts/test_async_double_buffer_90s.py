...
class AsyncDoubleBufferTest:
    ...
    def generate_comparison(self, sync: Dict, async_mode: Dict) -> Dict[str, Any]:
        ...
        if 'error' not in sync and 'error' not in async_mode:
            ...
            print(f"    稳定性: ±{sync.get('speed_std', 0)/sync_speed*100 if sync_speed > 0 else 0:.1f}%")
            ...
            print(f"    稳定性: ±{async_mode.get('speed_std', 0)/async_speed*100 if async_speed > 0 else 0:.1f}%")
