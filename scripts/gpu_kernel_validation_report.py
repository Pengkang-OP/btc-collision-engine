...
def main():
    ...
    # 测试10: .cl文件存在性
    # ========================================================================
    print_section("测试10: 独立.cl文件检查")
    
    try:
        kernel_cl = os.path.join(project_root, 'src', 'gpu', 'kernels', 'btc_collision.cl')
        readme_md = os.path.join(project_root, 'src', 'gpu', 'kernels', 'README.md')
        
        if os.path.exists(kernel_cl):
            size = os.path.getsize(kernel_cl)
            print_result("btc_collision.cl", True, f"{size:,} 字节")
            all_results.append(True)
        else:
            print_result("btc_collision.cl", False, "文件不存在")
            all_results.append(False)
        
        if os.path.exists(readme_md):
            print_result("README.md", True, "存在")
            all_results.append(True)
        else:
            print_result("README.md", False, "文件不存在")
            all_results.append(False)
            
    except Exception as e:
        print_result("文件检查", False, str(e))
        all_results.append(False)
