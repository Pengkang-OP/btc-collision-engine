...
def run_test(test_name, command, expected_output=None, should_fail=False):
    ...
    try:
        ...
        if expected_output:
            ...
            if "TIP" in expected_output:
                expected_variants.append("[Tip]")
            if "BOOKS" in expected_output:
                expected_variants.append("[Examples]")
            if "TOOL" in expected_output:
            ...
            if found:
                print("OK 通过 - 找到预期输出")
                return True
            print(f"ERR 失败 - 未找到预期输出: {expected_output}")
            print(f"实际输出:\n{output[:500]}")
            return False
        ...
        if should_fail:
            if result.returncode != 0:
                print(f"OK 通过 - 命令按预期失败 (退出码: {result.returncode})")
                return True
            print("ERR 失败 - 命令应该失败但成功了")
            return False
        if result.returncode == 0:
            print("OK 通过 - 命令成功执行")
            return True
        print(f"ERR 失败 - 命令执行失败 (退出码: {result.returncode})")
        print(f"输出:\n{output[:500]}")
        return False
