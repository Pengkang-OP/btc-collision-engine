...
REPLACEMENTS = [
    ...
    ("对于 512 位数 x = x_low + x_high * 2^256，有:", "For 512-bit x = x_low + x_high * 2^256:"),
    (
    ...
    (
        "输出: num_keys 个标志 (0=不匹配, target_index+1=匹配)",
        "Output: num_keys flags (0=no match, target_index+1=match)",
    ),
    (
        "预计算表: 31\u00d72\u00d78 = 496 uint32 (G1..G31仿射坐标)",
        "Precomputed table: 31x2x8 = 496 uint32 (G1..G31 affine)",
    ),
    (
        "预计算表: 31×2×8 = 496 uint32 (G1..G31仿射坐标)",
        "Precomputed table: 31x2x8 = 496 uint32 (G1..G31 affine)",
    ...
    (
        "读取私鑰 (uint32数组 -> uint256 小端)",
        "Read private key (uint32 array -> uint256 little-endian)",
    ),
    (
        "修复：每个私鑰现在是8个uint32（而非32个uchar）",
        "Fix: each private key is now 8 uint32 (not 32 uchar)",
    ),
