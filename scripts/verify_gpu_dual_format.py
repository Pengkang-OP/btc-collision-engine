...
def run_tests():
    ...
    seed_buf = cl.Buffer(
        ctx, cl.mem_flags.READ_ONLY | cl.mem_flags.COPY_HOST_PTR, hostbuf=seed_array
    )
    ...
    for test_name, target_h160, check_uncomp, should_match, desc in tests:
        ...
        targets_buf = cl.Buffer(
            ctx, cl.mem_flags.READ_ONLY | cl.mem_flags.COPY_HOST_PTR, hostbuf=target_array
        )
        num_targets = 1
    ...
    try:
        ...
        if local_mem_matched:
            ...
            print(
                f"   ❌ batch_check_local_mem: 非压缩目标匹配失败 (match_flags[0]={match_flags[0]})"
            )
