...
### 🔴 C2 - key_collision.py 导入失败（启动即崩溃）
...
### 🔴 C4 - 关键依赖包未在 requirements.txt 中声明
...
### 🟠 H5 - 配置参数 batch_size/num_workers 无上限验证
...
### 🟠 H8 - PyOpenCL 依赖在 GPU 不可用时缺少明确提示
...
### 🟠 H9 - collision_stats.py 边界情况数据不一致
...
|------|---------|--------|--------|---------|------|
| CLI 入口 | `key_collision_cli.py` | 95% | 完全可用 | 缺少顶层异常捕获（C5） | ⭐⭐⭐⭐⭐ |
| CPU 碰撞引擎 | `src/collision/` | 100% | 完全可用 | 无 | ⭐⭐⭐⭐⭐ |
| 加密核心 | `src/crypto/` | 100% | 完全可用 | 支持多级降级 | ⭐⭐⭐⭐⭐ |
| GPU 加速 | `src/gpu/` | 95% | 条件可用（需 PyOpenCL） | Intel 日志泄洪（H4） | ⭐⭐⭐⭐ |
| 监控系统 | `src/monitoring/` | 95% | 完全可用 | 统计边界情况（H9） | ⭐⭐⭐⭐⭐ |
| 配置管理 | `src/config/` | 100% | 完全可用 | 参数无上限验证（H5） | ⭐⭐⭐⭐⭐ |
| **GUI 界面** | **`src/gui/`（不存在）** | **0%** | **完全不可用** | **目录缺失（C1）** | ⭐ |
...
| 场景 | 当前处理 | 建议改进 |
|------|---------|---------|
| GPU 不可用 | 自动降级到 CPU ✅ | 添加明确提示消息 |
| 内存不足 | ❌ 未处理 | 捕获 MemoryError 并自动降低批大小 |
| 磁盘空间满 | ❌ 未处理 | 捕获 OSError 并给出提示 |
| 配置参数越界 | ❌ 无上限验证 | 添加 Schema 上限验证 |
| 用户 Ctrl+C 中断 | ⚠️ 部分处理 | 确保 CLI 入口捕获 KeyboardInterrupt |
| 加密后端不可用 | 多级降级 ✅ | 已实现完整降级链 |
| 非法配置文件 | ⚠️ 部分处理 | 提供更详细的错误定位信息 |
...
| 新用户安装失败率高 | 高 | 高 | 🔴 立即处理 |
| GUI 用户完全流失 | 高 | 高 | 🔴 立即处理 |
| CLI 崩溃时信息泄露 | 中 | 中 | 🟠 尽快处理 |
| Intel GPU 日志磁盘打满 | 中 | 中 | 🟠 尽快处理 |
| 配置越界导致系统崩溃 | 低 | 高 | 🟡 计划处理 |
